...
- Spec imports: use `require('../../../shared/fixtures')` (not `@playwright/test`) for new specs to enable self-healing
- Test data: all assertion strings/URLs must reference `const TD = require('../../../shared/data/test-data')` — never hardcode them inline
