  ...
  async generateTest(testDescription, options = {}) {
    ...
    try {
      ...
      if (await this.isMCPAvailable()) {
        ...
        try {
          // Check if it's JSON instead of JavaScript code
          if (code.trim().startsWith('{') && (code.includes('"Root Cause Analysis"') || 
              code.includes('"Specific Fix"') || code.includes('"testPlan"'))) {
            logger.error('[VALIDATION] ERROR: Generated JSON instead of JavaScript code!');
            throw new Error('MCP returned JSON analysis instead of executable test code');
          }

          // Check for required imports
          if (!code.includes('require(') && !code.includes('import ')) {
            logger.error('[VALIDATION] ERROR: Missing import statement');
            throw new Error('Generated code missing import statement (require or import)');
          }

          // Check for test() functions
          if (!code.includes('test(') && !code.includes('test.skip(') && !code.includes('test.only(')) {
            logger.error('[VALIDATION] ERROR: No test cases found in generated code');
            throw new Error('Generated code does not contain any test() functions');
          }

          // Check for invalid beforeAll/afterAll with fixtures (common error)
          const invalidHookPattern = /test\.(beforeAll|afterAll)\s*\(\s*async\s*\(\s*\{\s*\w+\s*\}\s*\)/;
          if (invalidHookPattern.test(code)) {
            logger.error('[VALIDATION] ERROR: Invalid hook syntax detected (fixtures not allowed in beforeAll/afterAll)');
            throw new Error('Generated code has invalid test.beforeAll/afterAll with fixtures - remove these hooks');
          }
          ...
          if (openBraces !== closeBraces) {
            logger.error(`[VALIDATION] Unbalanced braces: ${openBraces} open, ${closeBraces} close`);
            throw new Error(`Generated code has unbalanced braces (${openBraces} open, ${closeBraces} close)`);
          }
          if (openParens !== closeParens) {
            logger.error(`[VALIDATION] Unbalanced parentheses: ${openParens} open, ${closeParens} close`);
            throw new Error(`Generated code has unbalanced parentheses (${openParens} open, ${closeParens} close)`);
          }

          // Check for test.describe block
          if (!code.includes('test.describe(')) {
            logger.warn('[VALIDATION] Warning: Code missing test.describe() wrapper - may cause organizational issues');
          }

          logger.info('[VALIDATION] ✓ Generated code passed all validation checks');
        } catch (validationError) {
          logger.error(`[VALIDATION] Code validation failed: ${validationError.message}`);
          logger.error(`[VALIDATION] Code preview: ${code.substring(0, 200)}`);
          throw new Error(`Invalid code generated by MCP: ${validationError.message}`);
        }
