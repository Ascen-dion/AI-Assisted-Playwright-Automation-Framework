...
const HOMEPAGE_URL = 'https://www.endpointclinical.com/';
const INVALID_URL = 'https://www.endpointclinical.com/invalidpage';
const HEADLINE_SELECTOR = 'h1';
...
test.describe('Homepage Headline Tests', () => {
  ...
  test('Verify the presence of the headline on the homepage', async ({ page }) => {
    ...
    const headline = page.locator(HEADLINE_SELECTOR).first();
  ...
  test('Verify the absence of the headline when the homepage is not loaded', async ({ page }) => {
    await page.goto(INVALID_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
    const headline = page.locator(HEADLINE_SELECTOR).first();
