...
test.describe('[Mobile] QE-851 TS-004: Card management buttons disabled for locked card', { tag: ['@regression', '@mobile', '@card-management'] }, () => {
  ...
  test('[QE-851 TS-004 TC-003] Verify Report a Card Issue button is disabled for locked card', async ({ page }) => {
    ...
    // Step 5: Check the state of Report a Card Issue button
    const isReportCardIssueDisabled = await cardPage.isReportCardIssueButtonDisabled();
    expect(isReportCardIssueDisabled).toBeTruthy();
  });
});