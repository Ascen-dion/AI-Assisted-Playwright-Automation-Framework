/**
 * Test Spec: CAP1-1 TS-006 TC-003
 * Verify Workforce tab displays budget input sheet
 */
...
test.describe('[UI] CAP1-1 TS-006: Verify Budget Input Sheets', { tag: ['@regression', '@capital-one'] }, () => {
  ...
  test('[CAP1-1 TS-006 TC-003] Verify Workforce tab displays budget input sheet', async ({ page }) => {
    ...
    // Step 3: Click Workforce tab
    await budgetPage.clickTab('Workforce');

    // Step 4: Verify Workforce content is displayed
    const isActive = await budgetPage.isTabActive('Workforce');
