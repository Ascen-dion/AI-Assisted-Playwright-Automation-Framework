/**
 * Test Spec: CAP1-1 TS-006 TC-004
 * Verify Product Revenue tab displays budget input sheet
 */
...
test.describe('[UI] CAP1-1 TS-006: Verify Budget Input Sheets', { tag: ['@regression', '@capital-one'] }, () => {
  ...
  test('[CAP1-1 TS-006 TC-004] Verify Product Revenue tab displays budget input sheet', async ({ page }) => {
    ...
    // Step 3: Click Product Revenue tab
    await budgetPage.clickTab('Product Revenue');

    // Step 4: Verify Product Revenue content is displayed
    const isActive = await budgetPage.isTabActive('Product Revenue');
